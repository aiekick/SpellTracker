...
if (GetLocale() == "frFR") then
	...
	L["CLEAR_VIEW_DLG_INFOS"] = "Les donnÃ©es actuelles seront perdues !\nEtes vous sur de vouloir continuer ?";
    L["YES"] = "Oui";
    L["CANCEL"] = "Annuler";
	
	L["VIEWPLAYING"] = "Mise Ã  Jour de la Vue en Cours";
	L["VIEWPAUSE"] = "Mise Ã  Jour de la Vue en Pause";
	...
	L["SPT_DB_ERASE"] = "SpellTracker DB EffacÃ©e";
	L["TRACK_SRC"] = "Suivi de l'Ã©mission";
	L["TRACK_DST"] = "Suivi de la rÃ©ception";
	L["TRACK_ZONE"] = "Mode de Suivi de Zone par ";
	L["PLAYER_TRACKED"] = " Joueur Suivi : ";
	L["LeftMouseButton"] = "LMB :";
	L["RightMouseButton"] = "LMD :";
	L["INFOS_CUR_TRACK"] = "Infos Ã  propos du suivi actuel :";
	...
	L["TOOLTIP_MSG3"] = "Configurer la structure de donnÃ©e";
	L["TOOLTIP_MSG4"] = "Suivi de moi mÃªme";
	L["TOOLTIP_MSG5"] = "Suivi du joueur selectionnÃ©";
	...
	L["TOOLTIP_MSG9"] = "Enregistrement dÃ©marÃ©";
	L["TOOLTIP_MSG10"] = "Mise Ã  jour de la vue en pause (Peut etre automatique pour eviter les freeze pendant un combat)";
	L["TOOLTIP_MSG11"] = "Enregistrement StoppÃ©";
...
function SpellTrackerFrame_Init(self)
	...
	SpellTrackerDB.FILTEREDSPELLS = SpellTrackerDB.FILTEREDSPELLS or {}; -- version filtrés en fonction justement des filtres
	...
	-- indique les items possible pour la structure de donnée / 0.41
	SpellTrackerDB.Params.DataStructItems = 
	{
		[1]="SpellName", -- permet de pas être embêté avec un sort en 8... et son tick en 10... ( ce serait bien de faire le distinguo..; => a voir )
		[2]="HitType", -- le meme sort peux faire du heal et du dégat
		[3]="SourceClass", -- donne la class qui qui a êµ© dêµ¥cté¡°our les sort
		[4]="SourceType", -- Mob / Player / Pet / Vehicle
		[5]="SourceReaction", -- réaction de la source par rapport a mon perso
		[6]="SourceName", -- Name
		[7]="TargetClass", -- donne la class qui qui a êµ© dêµ¥cté¡°our les sort
		[8]="TargetType", -- Mob / Player / Pet / Vehicle
		[9]="TargetReaction", -- réaction de la cible par rapport a mon perso
		[10]="TargetName" -- Name
	};
	...
	-- par default => SpellTrackerDB.Params.DefaultCountSpellsToTrack = 50; -- pilote le nombre de barres qui vont être crées à l'avance
	-- un raid sur orgri ca fait 200 barres de sort principale
	--SpellTrackerDB.Params.DefaultCountSpellsToTrack = 200; -- pilote le nombre de barres qui vont être crées à l'avance
	
	SpellTrackerDB.SPELLSTOTTRACK = SpellTrackerDB.SPELLSTOTTRACK or {}; -- stock les sort à traquer
	
	SpellTrackerDB.CurrentPlayerTracked = SpellTrackerDB.CurrentPlayerTracked or {};
	if ( SpellTrackerDB.Tracking_Target ~= 3 ) then -- zone tracking c'est les 3 membre == nil ce qui est faususé par l'affectation + init de base truc = truc or machin
		...
		if ( ColumnFilterDlg and help ) then
			if ( ColumnFilterDlg.tmpArr ) then
				if ( ColumnFilterDlg.tmpArr[self.var.item] ~= nil ) then
					-- 0 == false / 1 == true / 2 == false + desactivÃ© / 3 == true + dÃ©sactivÃ©
	...
	if ( ColumnFilterDlg ) then
		if ( ColumnFilterDlg.tmpArr ) then
			...
			for k, v in pairs(ColumnFilterDlg.tmpArr) do
				...
				if ( v == 1 ) then -- 0 == false / 1 == true / 2 == false + desactivÃ© / 3 == true + dÃ©sactivÃ©
...
function SpellTrackerPopupColumnFilterDlgBtn_Update(help, scrollUpdate) -- ColumnFilter dlg view update / si scrollUpdate == true pas de maj data juste scrollvalue
	...
	if ( help and ColumnFilterDlg ) then
		if ( scrollUpdate == false or ColumnFilterDlg.tmpArr == nil ) then -- on regenere les donnÃ©es
			...
			for k, v in pairs(ColumnFilterDlg.tmpArr) do
				if ( k ~= "Count" ) then
					-- seulement les donnÃ©es active
					if ( v == 0 or v == 1 ) then -- 0 == false / 1 == true / 2 == false + desactivÃ© / 3 == true + dÃ©sactivÃ©
		...
		if ( ColumnFilterDlg.tmpArr ) then
			...
			for k, v in ipairs(ColumnFilterDlg.columnArr) do
				if ( k and v ) then
					if ( ColumnFilterDlg.tmpArr[v] == 0 or ColumnFilterDlg.tmpArr[v] == 1 ) then -- 0 == false / 1 == true / 2 == false + desactivÃ© / 3 == true + dÃ©sactivÃ©
						if ( (Count_Position) > ColumnFilterDlg.ScrollValue and (Count_Position) <= (ColumnFilterDlg.ScrollValue + ColumnFilterDlg.nBarDisplayed) ) then -- correspond a la position de la barre dê´©rê¥ comme cela on n'affiche que les barres qui sont affichable -- gain mê®¯ire donc
		...
		if ( k and v ) then
			if ( type(v) == "table" ) then
				if ( k ~= "QuickAcces" ) then
					for kk,vv in pairs(v) do
						if ( kk and vv == 1 ) then  -- 0 == false / 1 == true / 2 == false + desactivÃ© / 3 == true + dÃ©sactivÃ©
...
function SpellTracker:EndFrameResize() -- 0.53 
	...
	if ( SpellTracker:IsInCombat() == false ) then
		...
		SpellTracker:InitFrame(); -- on a redimentionné donc on reinit le layout	
...
function SpellTracker:InitDataStruct(dataStruct) -- 0.43 OK
	...
	if ( dataStruct ) then
		if ( type(dataStruct ) == "string" ) then
			...
			-- 0.41 / on extrait la structure de donnée et les filtres d'affichage
...
function SpellTracker:UpdateDisplayedFiterFromButtons() -- 0.43 simplification OK -- 0.42 -- met à jour SpellTrackerDB.Params.DisplayedFilters en fonction de l'état de selection des filtres -- 0.53 bug fix
	...
	-- 0.53 Bug fix : pour eviter qu'au changement de bouton on se retrouve de 1000 barre à 100 barre et en position 1000, on recale la scrollbar pour la dernier barre
...
function SpellTracker:CreateBar(frmName) -- 0.48 TreeIcon -- 0.51 bug fix about overlappiung of bar ( first white bar )
	...
	GuiBar.VAR.SpellTracked = false; -- si le spell est demandé a traquer
	GuiBar.VAR.Used = false; -- indique si la barre affiche des données actuellment ( remeit à false pendant le reset de la barre )
...
function SpellTracker:InitFrame()
	...
	if ( #SpellTracker.BARS < SpellTracker.CountBarDisplayed ) then
		local countBarToCreate = SpellTracker.CountBarDisplayed; --SpellTrackerDB.Params.DefaultCountSpellsToTrack; était défini par parametrage avant et coutait pas mal de mémoire du coup
		for k=0,countBarToCreate do 
			local frmName = "SpellTrackerBar"..tostring(idx);
			if ( _G[frmName] == nil ) then -- si la barre a pas encore été créé on la créé / permet de s'assurer qu'on aura tujours un nombre de barres suffisant et en corélation avec la fenetre
		...
		-- on montre celles qui doivent etre utilisée et on cache les autres
...
function SpellTracker:UpdateFilteredStructureAccordingToFilters() -- 0.57
	-- le but du jeu c'est de parcourir la bd et et des qu'on rentre dans une structure qui est visible dans la liste des filtre Ã  afficher on 
...
function SpellTracker:RecursExploringOfStructureAccordingToFilters(key, struct, heritedStruct, NodeMode, baseStruct, indexerOfParent) -- 0.41 / rempli ou met Ã  jour SpellTrackerDB.FILTEREDSPELLS en fonction des filtres d'affichage et de la strucutre de donnee -- 0.49 Tree Node
    ...
    if ( key and struct and heritedStruct ) then
        ...
        -- quand nodemode == 2 il faudrait inversÃ© index et indexerofparent
        -- SpellTrackerDB.FILTEREDSPELLS.mob.Nodes.Ilaris.Indexer = "Ilaris";
        -- SpellTrackerDB.FILTEREDSPELLS.mob.Nodes.Ilaris.IndexerOfParent = "REACTION_FRIENDLY@TrÃ©ant@HEAL@Toucher guÃ©risseur@player@REACTION_FRIENDLY";
        if ( NodeMode == 2 ) then
		...
		SpellTrackerDB.ColumnFilter[struct.help][key] = SpellTrackerDB.ColumnFilter[struct.help][key] or 0; -- 0 == false / 1 == true / 2 == false + desactivÃ© / 3 == true + dÃ©sactivÃ©
...
function SpellTracker:FillListToSortAndNodeToSortFromColumnFilter(ReComputeFiltering) -- recree la liste a afficher en fonction des filtre -- 0.47 ( calcul du temps d'execution de cette func ) -- 0.48 / Tree Node -- 0.72 bug fix on string.match( cCFilterString, spellStruct[v] )
	...
	if ( SpellTrackerDB.ColumnFilter.Count > 0 or ReComputeFiltering == true) then -- on met Ã  jour la liste des filtres de colonne
		-- on desactive tout en base // -- 0 == false / 1 == true / 2 == false + desactivÃ© / 3 == true + dÃ©sactivÃ©
		...
		-- on reactive que ce qui est dans la liste triÃ©s
...
function SpellTracker:UpdateFrame() -- 0.41 / affiche simplement mais ne recalcule aucune liste -- 0.48 Tree Node -- 0.49 Tree Node -- 0.50
	...
	-- declarer la fonction dans la portée de Count_Position et CountAllChilds permet de les garder en variables locales
	...
	function RecursUpdateNodes(Indexer)
		for k, v in pairs(SpellTrackerDB.NodesToSort[Indexer]) do -- 0.48
			if ( type(v) == "table" ) then
				...
				if ( (Count_Position) >= SpellTracker.ScrollValue and (Count_Position) < (SpellTracker.ScrollValue + SpellTracker.CountBarDisplayed) ) then -- correspond a la position de la barre désirée comme cela on n'affiche que les barres qui sont affichable -- gain mémoire donc
...
function SpellTracker:UpdateGuiBar(GuiBar, spellStruct, SpellListToSort_Position, ChildListToSort_Position, CountChild) -- 0.48 -- 0.55 type nodes -- 0.73 harmonisation des infos et display bug fix
	...
	if ( GuiBar and spellStruct ) then
		...
		local FirstNode = "Â²Â³Â¹";
		local MiddleNode = "Â¼Â½Â¹";		
		local LastNode = "Â²Â½Â¹";
		...
		if ( ChildListToSort_Position ) then -- 0.48 c'est un child
			TreeSymbolLabel = " "..tostring(ChildListToSort_Position).." "..TreeSymbolLabel; -- c (pour child 0.53 ) + numê³¯ de l'enfant 
			SpellNameLabel = SpellTracker:RepeatString(" ", string.len(TreeSymbolLabel)+3);
		else
			TreeSymbolLabel = " "..tostring(SpellListToSort_Position).." "..TreeSymbolLabel; -- numê³¯ de la barre
			SpellNameLabel = SpellTracker:RepeatString(" ", string.len(TreeSymbolLabel)+3);
		end; 
			...
			for i=0,#SpellTrackerDB.DataStruct do
				...
				if ( item ) then
					if ( string.match(SpellTrackerDB.Params.DisplayedFilters, item) ) then
						...
						elseif ( item=="SourceReaction" ) then -- réaction de la source par rapport a mon perso
						...
						elseif ( item=="TargetReaction" ) then -- réaction de la cible par rapport a mon perso
				...
				if ( item ) then
					if ( not string.match(SpellTrackerDB.Params.DisplayedFilters, item) ) then
						...
						elseif ( item == "SourceType" and SourceType ) then -- Mob / Player / Pet / Vehicle*
							SpellNameLabel = SpellNameLabel.." "..tostring(SourceType);
						elseif ( item=="SourceReaction" and SourceReaction ) then -- rê¢£tion de la source par rapport a mon perso
						...
						elseif ( item == "TargetType" and TargetType ) then -- Mob / Player / Pet / Vehicle
							SpellNameLabel = SpellNameLabel.." "..tostring(TargetType);
						elseif ( item=="TargetReaction" and TargetReaction ) then -- rê¢£tion de la cible par rapport a mon perso
...
-- AMMOUNT LINE
	-- On recupe les donnê¦³ Amount / 0.40
	local LocalStruct = {};
	if ( buffer ) then
		local StatBarColor = SpellTracker.StatusBarColors;
...
function SpellTracker:COMBAT_LOG_EVENT_UNFILTERED(_,DATETIME, EVENTTYPE, hideCaster, GUIDSRC, SRC, SRCFLAG1, SRCFLAG2, GUIDDST, DST, DSTFLAG1, DSTFLAG2, ...)
	...
	local ret = nil;
	-- on donne pas suite si le guid est pas présent dans le raid
	...
	-- on donne pas suite si le guid est pas présent dans le groupe
